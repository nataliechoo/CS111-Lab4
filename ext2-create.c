...
void write_inode_table(int fd) {
	...
	memcpy(&hello.i_block, name, strlen(name));
	hello.i_size = strlen(name);
...
void write_hello_world_file_block(int fd)
{
	...
	//set up offset, contents for the file, content size
	char const contents[] = "Hello world\n";
	off_t offset = BLOCK_OFFSET(HELLO_WORLD_FILE_BLOCKNO);


	//move offset to the file location
	offset = lseek(fd, offset, SEEK_SET);

	int err = write(fd, contents, sizeof(contents));
	if (err != sizeof(contents) || err == -1)
	{
		errno_exit("write");
	}
